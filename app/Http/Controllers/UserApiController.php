...
class UserApiController extends Controller {
    ...
    public function signup(Request $request) {
        ...
        try {
            ...
            $User['token'] = $token;
            $User['isVerified'] = ($request->social_unique_id == '') ? 0 : 1;
            $User['password'] = bcrypt($request->password);

            // if ($request->hasFile('avatar')) {
            //     foreach($request->file('avatar') as $image)
            //     {
            //         $url = $image->store('user/profile');
            //         $User['picture'] = $url;
            //     }
            // }
            ...
            return response()->json([
                        'message' => 'Lien de vérification envoyé à votre email!',
                        'user' => $User
            ]);
//            return $User;
        } catch (Exception $e) {
    ...
    public function change_password(Request $request) {
        ...
        if (Hash::check($request->old_password, $User->password)) {
            ...
            if ($request->ajax()) {
                ...
                return back()->with('flash_success', 'Mot de passe mis Ã  jour');
            }
        } else {
            if ($request->ajax()) {
    ...
    public function send_request(Request $request) {
        ...
        if ($ActiveRequests > 0) {
            if ($request->ajax()) {
                ...
                return redirect('dashboard')->with('flash_error', 'Demande est actuellement en cours. RÃ©essayez plus tard');
            }
        }
        ...
        if ($request->has('schedule_date') && $request->has('schedule_time')) {
            ...
            if (time() > strtotime($request->schedule_date . $request->schedule_time)) {
                if ($request->ajax()) {
                    ...
                    return redirect('dashboard')->with('flash_error', 'Impossible de crÃ©er une demande! RÃ©essayez plus tard');
                }
            }
            ...
            if ($CheckScheduling->count() > 0) {
                if ($request->ajax()) {
                    ...
                    return redirect('dashboard')->with('flash_error', 'DÃ©jÃ  la demande est prÃ©vue Ã  cette heure.');
                }
            }
        }
        ...
        if (count($Providers) == 0) {
            if ($request->ajax()) {
                ...
                return back()->with('flash_success', 'Aucun dÃ©panneur trouvÃ©! Veuillez rÃ©Ã©ssayer.');
            }
        }
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                return back()->with('flash_error', 'Quelque chose s\'est mal passÃ© lors de l\'envoi de la demande. Veuillez rÃ©essayer.');
            }
        }
    }
        ...
        try {
            ...
            if ($UserRequest->status == 'CANCELLED') {
                if ($request->ajax()) {
                    ...
                    return back()->with('flash_error', 'La demande est dÃ©jÃ  annulÃ©e!');
                }
            }
            ...
            if ($request->ajax()) {
                ...
                return back()->with('flash_error', 'Aucune demande trouvÃ©e!');
            }
        }
    }
    ...
    public function rate_provider(Request $request) {
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                return redirect('dashboard')->with('flash_success', 'DÃ©panneur Ã©valuÃ© avec succÃ¨s!');
            }
        } catch (Exception $e) {
            if ($request->ajax()) {
                return response()->json(['error' => trans('api.something_went_wrong')], 500);
            } else {
                return back()->with('flash_error', 'Quelque chose a mal tournÃ©');
            }
        }
    }
    ...
    public function add_promocode(Request $request) {
        ...
        $messages = [
            'promocode.exists' => 'Code promo invalide',
        ];
        $this->validate($request, [
        ...
        try {
            ...
            if ($find_promo->status == 'EXPIRED' || (date("Y-m-d") > $find_promo->expiration)) {
                ...
                if ($request->ajax()) {
                    ...
                    return back()->with('flash_error', 'Code promotionnel dÃ©jÃ  utilisÃ©');
                }
            } else {
    ...
    public function show_providers(Request $request) {
        ...
        try {
            ...
            if (count($Providers) == 0) {
                if ($request->ajax()) {
                    return response()->json(['message' => "Aucun dÃ©panneur trouvÃ©"]);
                } else {
                    return back()->with('flash_success', 'Aucun dÃ©panneur trouvÃ©! Veuillez rÃ©Ã©ssayer.');
                }
            }
    ...
    public function provider(Request $request) {
        ...
        if ($Provider = Provider::find($request->provider_id)) {
            ...
            return response()->json(['error' => 'Aucun dÃ©panneur trouvÃ©!'], 500);
        }
    }
    ...
    public function addService(Request $request) {
        ...
        return response()->json(['message' => 'DÃ©pannage ajoutÃ©']);
    }
